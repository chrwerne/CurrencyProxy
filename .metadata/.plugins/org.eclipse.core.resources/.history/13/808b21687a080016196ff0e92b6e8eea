...
public class Currencies extends HttpServlet {
	...
	private InforeurResult callInforeur(String language, String year, String month) {
		...
		try {
			...
			urlString = urlString.replace("##lang##", language);
			...
			if (conn.getResponseCode() == 200) {
				logger.debug("Returncode from Inforeur <"+conn.getResponseCode()+">");
			...
			logger.error("IOException when connecting to Inforeur - maybe wrong parameters...");
		}
